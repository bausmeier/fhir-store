...
tap.test('Practitioner query builder', async (t) => {
  t.test('should match on resource type when there are no parameters', async (t) => {
    const expectedQuery = {
  ...
  t.test('should add a filter for the identifier query parameter with a namespace', async (t) => {
    const expectedQuery = {
      resourceType: 'Practitioner',
      identifier: {
        $elemMatch: {
