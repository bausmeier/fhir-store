...
tap.test('Observation query builder', async t => {
  ...
  t.test('should add a regex filter for the id subject parameter', async t => {
    ...
    t.ok(
      filters['subject.reference'].test('http://localhost/Group/123/_history/1')
    )
    ...
    t.ok(
      filters['subject.reference'].test(
        'http://localhost/Device/123/_history/1'
      )
    )
    ...
    t.ok(
      filters['subject.reference'].test(
        'http://localhost/Location/123/_history/1'
      )
    )
  })

  t.test(
    'should add a regex filter for the id subject parameter by type',
    async t => {
      const filters = buildObservationQuery({
        'subject:Patient': '123'
      })
      ...
      t.notOk(
        filters['subject.reference'].test(
          'http://localhost/Group/123/_history/1'
        )
      )
      ...
      t.notOk(
        filters['subject.reference'].test('http://localhost/Location/123')
      )
      t.notOk(
        filters['subject.reference'].test(
          'http://localhost/Location/123/_history/1'
  ...
  t.test(
    'should add a filter for the name query parameter when no system is provided',
    async t => {
      const expectedFilters = {
        resourceType: 'Observation',
        $and: [
  ...
  t.test(
    'should add a filter for the name query parameter when a system is provided',
    async t => {
      const expectedFilters = {
        resourceType: 'Observation',
        $and: [
  ...
  t.test(
    'should add a filter for the name query parameter when multiple codes are provided',
    async t => {
      const expectedFilters = {
        resourceType: 'Observation',
        $and: [
  ...
  t.test(
    'should add a filter for the name query parameter when multiple codes and systems are provided',
    async t => {
      const expectedFilters = {
        resourceType: 'Observation',
        $and: [
