...
tap.test('Transaction', async t => {
  ...
  t.test(
    'should return an empty bundle when there are no entries in the transaction',
    async t => {
      ...
      bundleCreator.createBundle
        .withArgs('http://localhost/', 'Transaction Results', sinon.match([]))
  ...
  t.test(
    'should return a validation error when there are entries without ids',
    async t => {
      const transaction = {
        entry: [
          {
  ...
  t.test(
    'should return a validation error when there are entries with transient ids',
    async t => {
      const transaction = {
        entry: [
          {
  ...
  t.test(
    'should return a validation error when there are entries with invalid an resource',
    async t => {
      const transaction = {
        entry: [
          {
      ...
      try {
        ...
        t.equal(
          err.message,
          'Invalid resource in entry http://localhost/Patient/1'
        )
      }
    }
  )
  ...
  t.test(
    'should return a resource bundle when the transaction is successful',
    async t => {
      const resources = [
        {
          id: '1',
      ...
      repo.updateResources
        .withArgs(resources.map(sinon.match))
        .resolves(resources)
