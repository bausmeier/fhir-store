...
tap.test('Search', async t => {
  ...
  t.test('should correctly construct bundle', async t => {
    repo.searchResources
      .withArgs('Patient', sinon.match({}))
      .resolves({resources: [], count: 0})

    const expectedBundle = generateBundle()

    bundleCreator.createBundle
      .withArgs('http://localhost/', 'Search Results', sinon.match([]), 0)
  ...
  t.test('should pass through query options to repo', async t => {
    ...
    repo.searchResources
      .withArgs('Practitioner', sinon.match(query))
      .resolves({resources: [], count: 0})
    ...
    t.equal(
      self.href,
      'http://localhost/Practitioner?_count=10&page=2&identifier=16cd9ef0-e9a9-48a1-b015-b40923e17ddc'
    )
  })

  t.test('should handle errors from repo', async t => {
    repo.searchResources
      .withArgs('Encounter', sinon.match({}))
