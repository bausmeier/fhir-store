...
tap.test(
  ...
  common.testWithRepo(async (t, repo) => {
    t.test(
      'should return a not found error if the resource is not found',
      async t => {
        const id = uuid()
        try {
    ...
    t.test(
      'should return a deleted error if the resource has been deleted',
      async t => {
        ...
        try {
          ...
          t.equal(
            err.message,
            `Patient with id ${resource.id} has been deleted`
          )
        }
      }
    )
    ...
    t.test('should return the resource if it exists', async t => {
      ...
      const returnedResource = await repo.findResource(
