...
tap.test(
  ...
  common.testWithRepo(async (t, repo) => {
    t.test('should create the resource if it does not exist', async t => {
      ...
      const updatedResource = await repo._db
    ...
    t.test('should update the resource if it exists', async t => {
      ...
      const resource = Object.assign(common.generatePatient(), {
    ...
    t.test(
      "should insert the updated resource into the 'versions' collection",
      async t => {
        const resource = common.generatePatient()
    ...
    t.test(
      'should not create the resource if the ifMatch option is *',
      async t => {
        const resource = common.generatePatient()
        try {
    ...
    t.test(
      'should update the resource if the ifMatch option matches the version id',
      async t => {
        const existingResource = common.generatePatient()
        await repo._db.collection('resources').insertOne(existingResource)
        ...
        const {resource: returnedResource} = await repo.updateResource(
          resource,
          {ifMatch: existingResource.meta.versionId}
        )
        t.deepEqual(returnedResource, resource)
    ...
    t.test(
      'should return a conflict error if the ifMatch option does not match the version id',
      async t => {
        const resource = common.generatePatient()
        try {
