...
tap.test('updateResources', common.testWithRepo((t, repo) => {
  t.test('should create the resources if they don\'t exist', async (t) => {
    ...
    const updatedResources = await repo._db.collection('resources').find(query, {projection: {_id: 0}}).sort({_id: 1}).toArray()
  ...
  t.test('should insert the resources into the \'versions\' collection', async (t) => {
    const resources = [
      common.generatePatient(),
      common.generatePatient()
    ]
