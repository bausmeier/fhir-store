...
tap.test(
  ...
  common.testWithRepo(async (t, repo) => {
    await repo._db.collection('resources').deleteMany({resourceType: 'Patient'})
    ...
    t.test(
      'should return all patients when no query parameters are specified',
      async t => {
    ...
    t.test(
      'should return the correct patients when searching by identifier',
      async t => {
        const expectedResources = existingResources.filter(resource => {
          return (
    ...
    t.test(
      'should return the correct patients when searching by identifier and system',
      async t => {
        const expectedResources = existingResources.filter(resource => {
          return (
            resource.resourceType === 'Patient' &&
            ...
            resource.identifier.some(
              id => id.value === identifier && id.system === system
            )
          )
        })
        const {resources: returnedResources} = await repo.searchResources(
