...
const {ConflictError, DeletedError, NotFoundError} = require('fhir-errors')
...
class Repo {
  ...
  async initialise() {
    ...
    this._client = await MongoClient.connect(
      this._options.url,
      Object.assign({}, DEFAULT_DB_OPTIONS, this._options.options)
    )
    ...
    try {
      ...
      if (err.code === 11000) {
        throw new ConflictError(
          `${resource.resourceType} with id ${resource.id} already exists`
        )
      }
      throw err
    }
  ...
  async updateResource(resource, options) {
    ...
    const result = await this._db
      ...
      .findOneAndReplace(filter, resource, queryOptions)
    if (!result.value) {
      throw new ConflictError(
        `${resource.resourceType} with id ${resource.id} does not match`
      )
    }
  ...
  async findResource(resourceType, id) {
    ...
    const resource = await this._db
      .collection('resources')
      .findOne(query, {projection: DEFAULT_PROJECTION})
    if (resource != null) {
      return resource
    }
