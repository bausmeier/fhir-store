...
class Repo {
  ...
  findResource (resourceType, id, callback) {
    ...
    this._db.collection('resources').findOne(query, DEFAULT_PROJECTION, (err, resource) => {
      if (err) {
        return callback(err)
      }
  ...
  findResourceVersion (resourceType, id, version, callback) {
    ...
    this._db.collection('versions').findOne(query, DEFAULT_PROJECTION, callback)
  }
  ...
  searchResources (resourceType, parameters, callback) {
    ...
    function findResources (callback) {
      if (Array.isArray(query)) {
        collection.aggregate(query, callback)
      } else {
        const cursor = collection.find(query, DEFAULT_PROJECTION).sort(DEFAULT_SORT)
